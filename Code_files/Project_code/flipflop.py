...
fill = [2] * N
ranges = array('i', fill)
outfile = './FinalFF/FLIPFLOP850_@ALG@_@N@_LOG.csv'
...
for t in range(numTrials):
	for CE in [0.15,0.35,0.55,0.75,0.95]:
		fname = outfile.replace('@ALG@','SA_{}'.format(CE)).replace('@N@',str(t+1))
		with open(fname,'w') as f:
			f.write('iterations,fitness,time,fevals\n')
		ef = FlipFlopEvaluationFunction()
		odd = DiscreteUniformDistribution(ranges)
...
for t in range(numTrials):
	for pop,mate,mutate in product([100],[50,30,10],[50,30,10]):
		fname = outfile.replace('@ALG@','GA_{}_{}_{}'.format(pop,mate,mutate)).replace('@N@',str(t+1))
		with open(fname,'w') as f:
			f.write('iterations,fitness,time,fevals\n')
		ef = FlipFlopEvaluationFunction()
		odd = DiscreteUniformDistribution(ranges)
...
for t in range(numTrials):
	for samples,keep,m in product([100],[50],[0.1,0.3,0.5,0.7,0.9]):
		fname = outfile.replace('@ALG@','MIMIC_{}_{}_{}'.format(samples,keep,m)).replace('@N@',str(t+1))
		with open(fname,'w') as f:
			f.write('iterations,fitness,time,fevals\n')
		ef = FlipFlopEvaluationFunction()
		odd = DiscreteUniformDistribution(ranges)
